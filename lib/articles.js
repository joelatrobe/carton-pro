...
/* One store shape, two collections: articles and vacancies. */
function store(dataDir, filename) {
  const file = path.join(dataDir, filename || 'articles.json');
...
/* headingTag lets a vacancy card, whose own title is an h3, keep its
   subheadings below it in the outline rather than jumping back up to h2. */
function renderBody(body, headingTag) {
  const tag = headingTag === 'h4' ? 'h4' : 'h2';
  ...
  blocks.forEach((raw) => {
    ...
    if (block.startsWith('## ')) {
      out.push(`<${tag} class="subhead u-mt-l">${escapeHtml(block.slice(3).trim())}</${tag}>`);
      return;
    }
